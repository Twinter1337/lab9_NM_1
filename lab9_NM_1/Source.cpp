...
double* Find_Coeff(double** pol_def, double** free_member_col_def, int row, int col) {
	...
	while (true) {
		for (int i = 0; i < col; i++) {
			...
			cram_matr[i][0] = free_member_col_def[i][0];
		}
		...
		for (int i = 0; i < col; i++) {
			...
			cram_matr[i][1] = free_member_col_def[i][0];
		}
		...
		for (int i = 0; i < col; i++) {
			...
			cram_matr[i][2] = free_member_col_def[i][0];
		}
		...
		for (int i = 0; i < col; i++) {
			...
			cram_matr[i][3] = free_member_col_def[i][0];
		}
...
double DetMatr3x3(double** matr) {
	double det = matr[0][0] * matr[1][1] * matr[2][2] + matr[0][1] * matr[1][2] * matr[2][0] + matr[1][0] * matr[2][1] * matr[0][2]
